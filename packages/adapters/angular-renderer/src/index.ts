...
export interface AngularAdapterContext {
...
export type AngularAdapterRenderFn = (component: UIComponent, ctx: AngularAdapterContext) => string;

export interface AngularAdapterRegistry {
  register: (prefix: string, render: AngularAdapterRenderFn) => void;
  resolve: (adapterHint: string) => AngularAdapterRenderFn | undefined;
  hasPrefix: (prefix: string) => boolean;
  listPrefixes: () => string[];
}

export interface RenderPageAngularOptions {
  uiSchema: UISchema;
  data: Record<string, JSONValue>;
  context: ExecutionContext;
  i18n?: I18nProvider;
...
export interface RenderPageAngularResult {
  html: string;
  data: Record<string, JSONValue>;
  context: ExecutionContext;
...
export interface AngularHydrationSession {
  dispose: () => void;
  rerender: () => void;
  dispatcher: RendererEventDispatcher;
}
...
const bootstrapped = new WeakSet<AngularAdapterRegistry>();

export function createAdapterRegistry(): AngularAdapterRegistry {
  const entries: Array<{ prefix: string; render: AngularAdapterRenderFn }> = [];
  return {
...
export function registerAdapter(
  prefix: string,
  render: AngularAdapterRenderFn,
  adapterRegistry: AngularAdapterRegistry = defaultAdapterRegistry,
): void {
  adapterRegistry.register(prefix, render);
}

export function getDefaultAdapterRegistry(): AngularAdapterRegistry {
...
export function RenderPageAngular(options: RenderPageAngularOptions): RenderPageAngularResult {
    ...
    if (input.event === 'onChange') {
      ...
      if (parsed) {
        if (parsed.target === 'data') runtime.data = setPath(runtime.data, parsed.path, value);
          ...
          runtime.context = setPath(
            runtime.context as unknown as Record<string, JSONValue>,
            parsed.path,
            value,
      ...
      runtime.options.onAdapterEvent?.(
        'onChange',
        {
  ...
  const dispatcher = createAngularEventDispatcher(dispatchCore);

  const response: RenderPageAngularResult = {
    html: runtime.html,
    data: runtime.data,
...
export function renderAngular(options: AngularRenderOptions): string {
...
function renderRuntime(runtime: Runtime): string {
  ...
  const renderComponent = (
    componentId: string,
    override?: Pick<UIGridItem, 'props' | 'bindings' | 'rules'>,
  ): string => {
    ...
    const html = adapter(component, {
      ...
      ruleState,
      events: {
        onChangeAttrs: (bindingPath) =>
          eventAttrs('onChange', component.id, bindingPath ?? component.bindings?.data?.value),
  ...
  const renderLayout = (node: LayoutNode): string => {
    ...
    if (node.type === 'grid') {
      return `<div data-layout="grid" style="${buildLayoutStyle(node, runtime.i18n.direction)}">${components}${children}</div>`;
    }
    if (node.type === 'stack') {
      return `<div data-layout="stack" style="${buildLayoutStyle(node, runtime.i18n.direction)}">${components}${children}</div>`;
    }
    if (node.type === 'tabs') {
...
function ensureDefaultAdapters(adapterRegistry: AngularAdapterRegistry): void {
    ...
    (component, ctx) => {
      ...
      if (component.adapterHint === 'platform.textField') {
        const label = component.i18n?.labelKey
          ...
          : String(component.props?.label ?? 'Text field');
        ...
        return `<select ${ctx.events.onChangeAttrs()} aria-label="${escapeHtml(aria)}" ${ctx.ruleState.disabled ? 'disabled' : ''}>${options
          .map((opt) =>
            typeof opt === 'object' && opt && 'value' in opt
              ? `<option value="${escapeHtml(String((opt as { value: unknown }).value ?? ''))}">${escapeHtml(String((opt as { label?: unknown }).label ?? (opt as { value: unknown }).value ?? ''))}</option>`
              : '',
          )
          .join('')}</select>`;
      }
      if (component.adapterHint === 'platform.section') {
        const title = component.i18n?.labelKey
          ? ctx.i18n.t(component.i18n.labelKey)
          : String(component.props?.label ?? 'Section');
      ...
      if (component.adapterHint === 'platform.table') {
        ...
        const fields = cols
          .map((col) =>
            typeof col === 'object' && col && 'field' in col
              ? String((col as { field: unknown }).field ?? '')
              : '',
          )
          .filter(Boolean);
        return `<table aria-label="${escapeHtml(aria)}"><thead><tr>${fields
          ...
          .map(
            (row) =>
              `<tr>${fields
                .map(
                  (field) =>
                    `<td>${escapeHtml(String((row as Record<string, unknown>)[field] ?? ''))}</td>`,
                )
                .join('')}</tr>`,
          )
          .join('')}</tbody></table>`;
  ...
  registerAdapter(
    'material.',
    (component, ctx) =>
      adapterRegistry.resolve('platform.')!(
        {
          ...component,
          adapterHint:
            component.adapterHint === 'material.button' ? 'platform.button' : 'platform.textField',
        },
        ctx,
      ),
    adapterRegistry,
  ...
  registerAdapter(
    'aggrid.',
    (component, ctx) =>
      adapterRegistry.resolve('platform.')!({ ...component, adapterHint: 'platform.table' }, ctx),
    adapterRegistry,
  );
}
...
function applySetValueRule(
  ...
  const parsed = parseBindingPath(
    rule.path ?? component.bindings?.data?.value ?? `data.${component.id}`,
    'data',
  );
...
function readBindingGroup(
  ...
  for (const [key, raw] of Object.entries(source)) {
    ...
    if (!parsed) continue;
    const target =
      parsed.target === 'context' ? (context as unknown as Record<string, JSONValue>) : data;
    out[key] = {
...
export function createAngularEventDispatcher(
  dispatchEvent: (input: {
    event: UIEventName;
    componentId: string;
    value?: JSONValue;
...
export function bindAngularDomListeners(options: {
  target: HTMLElement;
  dispatcher: RendererEventDispatcher;
  rerender: () => void;
}): () => void {
...
export function hydrateAngular(options: {
  result: RenderPageAngularResult;
  target: HTMLElement | string;
}): AngularHydrationSession {
  const target = resolveTarget(options.target);
  if (!target) {
    throw new Error('Hydration target could not be resolved.');
  }
  target.innerHTML = options.result.html;
  const rerender = () => {
    target.innerHTML = options.result.html;
  };
  const dispose = bindAngularDomListeners({
    target,
