...
describe('vue-renderer', () => {
  ...
  it('hydrates rendered html and auto-binds dom listeners', () => {
    ...
    const session = hydrateVue({ result, target });
  ...
  it('renders flex and nested grid layouts with responsive hints', () => {
    ...
    const html = renderVue({ uiSchema: layoutSchema, data: buildData(), context });
  ...
  it('renders RTL direction and flips row layout for RTL locales', () => {
    ...
    const html = renderVue({ uiSchema: layoutSchema, data: buildData(), context: { ...context, locale: 'ar' }, i18n: rtlI18n });
