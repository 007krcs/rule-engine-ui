...
describe('layout-rendering', () => {
  it('renders stack layout with flexbox properties', () => {
    ...
    const html = renderVue({ uiSchema: stackSchema, data: {}, context });
  ...
  it('renders grid layout with columns', () => {
    ...
    const html = renderVue({ uiSchema: gridSchema, data: {}, context });
  ...
  it('renders section with flexbox when enabled', () => {
    ...
    const html = renderVue({ uiSchema: sectionSchema, data: {}, context });
  ...
  it('renders responsive grid with breakpoints', () => {
    ...
    const desktopHtml = renderVue({ uiSchema: gridSchema, data: {}, context });
    expect(desktopHtml).toContain('grid-template-columns:repeat(12,minmax(0,1fr))');
    expect(desktopHtml).toContain('gap:12px');

    const mobileHtml = renderVue({ uiSchema: gridSchema, data: {}, context: mobileContext });
  ...
  it('renders tabs with tab IDs', () => {
    ...
    const html = renderVue({ uiSchema: tabsSchema, data: {}, context });
...
describe('cross-framework-consistency', () => {
  it('produces consistent output between Vue and Angular renderers for same schema', () => {
    const vueHtml = renderVue({ uiSchema: schema, data: buildData(), context });
    // We test that the key structural elements are the same
    expect(vueHtml).toContain('data-ui-page="portable-page"');
    expect(vueHtml).toContain('data-layout="section"');
    expect(vueHtml).toContain('data-rf-component-id="nameField"');
    expect(vueHtml).toContain('data-rf-component-id="roleSelect"');
    expect(vueHtml).toContain('data-rf-component-id="saveButton"');
    expect(vueHtml).toContain('data-rf-component-id="summaryTable"');
    expect(vueHtml).toContain('data-rf-event="onChange"');
    expect(vueHtml).toContain('data-rf-event="onClick"');
  });
});
